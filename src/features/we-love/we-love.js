...
} from '../../core/app.js?v=4.2.85';
import * as sync from '../../core/sync.js?v=4.2.85';
import { encrypt, decrypt } from '../../core/crypto.js?v=4.2.85';
import { updateSidebarNavVisibility } from '../thu-chi-doi-ngoai/thu-chi.js?v=4.2.85';
...
function getAudioInstance() {
    if (!weLoveAudio) {
        weLoveAudio = new Audio('./mot-doi.mp3?v=4.2.85');
...
function initMediaSession() {
    ...
    if ('mediaSession' in navigator && aud) {
        const logoPath = './logo_pwa_small.png?v=4.2.85';
...
export async function logSpouseVisit() {
    ...
    const isAdmin = state.ownerEmail && user.email.toLowerCase().trim() === state.ownerEmail.toLowerCase().trim();
    if (!isAdmin) {
...
export async function renderWeLoveDashboard() {
    ...
    const isSpouseRole = !!state.spouseEmail && state.viewingSharedFund;
