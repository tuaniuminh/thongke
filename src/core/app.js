...
} from '../features/thu-chi-doi-ngoai/thu-chi.js?v=4.3.275';
import { initHealthBindings, renderHealthDashboard, updateProfileDropdowns } from '../features/ho-so-y-te/ho-so-y-te.js?v=4.3.275';
import { initFundBindings, renderFundDashboard, renderManagementTab } from '../features/quy-gia-dinh/quy-gia-dinh.js?v=4.3.275';
import { checkNewMonthNotification } from '../features/quy-gia-dinh/bao-cao-thang.js?v=4.3.275';
// app.js - Main Application Logic & UI Control 
import { encrypt, decrypt, generateAsymmetricKeypair, encryptWithPublicKey, decryptWithPrivateKey } from './crypto.js?v=4.3.275';
import * as sync from './sync.js?v=4.3.275';
import { updateHomeWeather } from '../features/thoi-tiet/thoi-tiet.js?v=4.3.275';
import { initWeLoveBindings, renderWeLoveDashboard, updateHomeLoveWidget, updateLoveWidgetUI } from '../features/we-love/we-love.js?v=4.3.275';
import { initLunarCalendarBindings, getDayStatus, isSatChuDay } from '../features/am-lich/am-lich.js?v=4.3.275';
import { initMotoCare, switchMotocareView } from '../features/motocare/motocare.js?v=4.3.275';
import { checkForUpdates, showUpdateModal, detectPlatform, exportAndShareFile } from './updater.js?v=4.3.275';
import { appLock } from '../features/app-lock/app-lock.js?v=4.3.275';

const APP_VERSION = '4.3.275';
...
function initTauriTitlebar() {
    const isTauriEnv = !!(window.__TAURI__ || window.__TAURI_IPC__ || window.__TAURI_METADATA__ || window.location.protocol === 'tauri:' || (window.location.protocol === 'https:' && window.location.hostname === 'tauri.localhost'));
    
    if (isTauriEnv) {
        document.documentElement.classList.add('is-tauri');
        if (document.body) document.body.classList.add('is-tauri');
    }
    ...
    if (!setupTauriWindowControls()) {
        setTimeout(setupTauriWindowControls, 100);
        setTimeout(setupTauriWindowControls, 500);
        setTimeout(setupTauriWindowControls, 1500);
    }
}
...
async function performSync(silent = false) {
    ...
    try {
            ...
            try {
                ...
                if (remoteResetTime > localResetTime || !localReset) {
                    ...
                    if (isUnlinkingActive || state.spouseStatus === 'left' || remoteData.spouseStatus === 'left' || isLocalUnlinked) {
                        ...
                        if (remoteData) {
                            remoteData.spouseEmail = '';
                            remoteData.spouseStatus = '';
                            remoteData.spouseRole = 'wife';
                            remoteData.viewingSharedFund = false;
                            remoteData.sharedFundOwnerEmail = '';
                            remoteData.pairingCodeAccepted = '';
                        }
                        state.spouseEmail = '';
                        state.spouseStatus = '';
                        state.spouseRole = 'wife';
                        ...
                        if (remoteData.spouseStatus === 'accepted' && remoteData.spouseEmail) {
        ...
        // 5d. Encrypt Fund Key for spouse (if configured and we are the Owner)
        if (state.spouseEmail && !state.viewingSharedFund) {
...
function updateThemeUI() {
    ...
    if (activeT === 'welove' || activeT === 'welove-admin' || activeT === 'welove-settings') {
        if (state.theme === 'light') {
            updateThemeColorMeta('#fff1f2');
        } else {
            updateThemeColorMeta('#1e1b4b');
        }
    } else {
        if (state.theme === 'light') {
...
function switchTab(tabId, updateHash = true, pushHistory = true) {
    ...
    if (sidebar) {
        if (tabId === 'health' || tabId === 'health-reminders' || tabId === 'welove' || tabId === 'welove-admin' || tabId === 'welove-settings') {
    ...
    if (tabId === 'dashboard') {
        ...
        // Render cổng ghép đôi gia đình (tập trung tại tab cài đặt)
        if (typeof window.renderFamilyPairingSettings === 'function') {
...
async function initializeApp() {
    ...
    // Custom JS swipe gestures disabled by developer to prevent conflicts.
    // Navigation on this Capacitor app container will be controlled via buttons/UI elements.
    ...
    // Bind Home Page Card clicks with Bug Detector logs
    const homeFinance = document.getElementById('homeCardFinance');
    if (homeFinance) {
        homeFinance.addEventListener('click', (e) => {
            console.log('[BUG DETECTOR] Clicked homeCardFinance element -> switchTab(dashboard)');
            switchTab('dashboard');
        });
    }
    const homeHealth = document.getElementById('homeCardHealth');
    if (homeHealth) {
        homeHealth.addEventListener('click', (e) => {
            console.log('[BUG DETECTOR] Clicked homeCardHealth element -> switchTab(health)');
            switchTab('health');
        });
    }
    const homeFund = document.getElementById('homeCardFund');
    if (homeFund) {
        homeFund.addEventListener('click', (e) => {
            console.log('[BUG DETECTOR] Clicked homeCardFund element -> switchTab(fund)');
            switchTab('fund');
        });
    }
    ...
    if (toggleShowLoveWidget) {
        toggleShowLoveWidget.addEventListener('change', async (e) => {
            state.showLoveWidget = e.target.checked;
            state.showLoveWidgetUpdated = new Date().toISOString();
            await saveLocalState();
            updateLoveWidgetUI();
...
function resetViewportZoom() {
    ...
    if (el) {
        el.setAttribute('content', 'width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover');
        setTimeout(() => {
            el.setAttribute('content', 'width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover');
        }, 100);
    }
}
...
function updateHomeLunar() {
    ...
    const lunarDateText = `${dayStr}/${monthStr}/${lunar.lYear}${leapText}`;
...
window.runLayoutDiagnostics = function() {
    ...
    // Log computed style of body::before pseudo-element
    const bodyBeforeStyle = window.getComputedStyle(document.body, '::before');
    const beforeBgImg = bodyBeforeStyle.backgroundImage || 'none';
    console.log(`[LayoutDiag] Element [body::before]: ` +
        `display=${bodyBeforeStyle.display}, ` +
        `position=${bodyBeforeStyle.position}, ` +
        `z-index=${bodyBeforeStyle.zIndex}, ` +
        `background-image="${beforeBgImg.length > 50 ? beforeBgImg.substring(0, 50) + '...' : beforeBgImg}"`);
...
// ============================================================
// --- DYNAMIC CARD NAVBAR REGISTRY (PROPOSAL #1) ---
const CARD_NAV_REGISTRY = {
    'thuchi': {
        logoText: 'Thu Chi Đối Ngoại',
        logoThemeClass: 'theme-thuchi',
        allowedNavs: ['home', 'dashboard', 'received', 'sent', 'tc-management', 'settings']
    },
    'health': {
        logoText: 'Hồ Sơ Y Tế',
        logoThemeClass: 'theme-health',
        allowedNavs: ['home', 'health', 'health-reminders', 'settings']
    },
    'welove': {
        logoText: 'Kỷ Niệm Tình Yêu',
        logoThemeClass: 'theme-welove',
        allowedNavs: ['home', 'welove', 'welove-admin', 'welove-settings', 'settings']
    },
    'fund': {
        logoText: 'Quỹ Gia Đình',
...
function getCardKeyForTab(tabId) {
...
function updateSidebarNavVisibility(tabId) {
    ...
    const cardKey = getCardKeyForTab(tabId);
    const cardConfig = CARD_NAV_REGISTRY[cardKey];

    if (sidebarLogoText) {
        if (cardConfig) {
            sidebarLogoText.innerText = cardConfig.logoText;
            sidebarLogoText.className = `logo-text ${cardConfig.logoThemeClass}`;
        } else {
    ...
    // Dynamic nav item visibility controller (Tối ưu biệt lập 100%)
    const allNavElements = document.querySelectorAll('[data-nav]');
    const allowed = cardConfig ? cardConfig.allowedNavs : ['home', 'settings'];

    allNavElements.forEach(el => {
        const navKey = el.getAttribute('data-nav');
        // Quyền riêng tư đối với tài khoản Vợ (Spouse) trong WeLove
        if (state.viewingSharedFund && (navKey === 'welove-admin' || navKey === 'welove-settings')) {
...
function updateMobileNavbar(tabId) {
    ...
    if (tabId === 'settings') {
        ...
        const curSubView = window.weLoveCurrentSubView || (tabId === 'welove-admin' ? 'admin' : tabId === 'welove-settings' ? 'settings' : 'memory');
        ...
        mobileNavbar.innerHTML = `
            <div class="mobile-navbar-left" style="width: 100%; justify-content: space-between !important; display: flex; align-items: center;">
                <div onclick="if(window.switchMotocareView) window.switchMotocareView('dashboard')" style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                    ...
                    <span class="mobile-navbar-title theme-motocare" id="mobileNavbarTitle">Chăm Sóc Xe</span>
                </div>
                <button class="nav-icon-btn text-below" onclick="window.navigateToTab('home')" title="Trang chủ">
                    <i data-lucide="home"></i>
                    <span class="btn-label">Trang chủ</span>
...
export { logScrollDiagnostics, triggerHapticFeedback, initNotificationSettingsUI, saveNotificationSettings, testNotificationWebhook, initDesktopHomeLayoutSettingsUI, setDesktopHomeLayoutMode };
