...
public class LiveActivityPlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    @objc public func downloadAndOpenIPA(_ call: CAPPluginCall) {
        ...
        let fileName = url.lastPathComponent.isEmpty ? "FamiLife-Update.ipa" : url.lastPathComponent
        ...
        self.activeDownloader?.onCompletion = { [weak self] localUrl, error in
            if let error = error {
                DispatchQueue.main.async { call.reject("Lỗi tải file: \(error.localizedDescription)") }
                return
            }
            guard let localUrl = localUrl else {
                DispatchQueue.main.async { call.reject("Không tìm thấy file vừa tải") }
                return
            }
