...
const EVENT_TYPES = ['Tất cả sự kiện', 'Đám cưới', 'Đám hiếu', 'Thăm ốm', 'Tân gia', 'Khác'];

export default function SentList({ navigation }) {
  const { state } = useApp();
  const { sentGifts = [] } = state;
  ...
  // Filter logic
  const activeGifts = sentGifts.filter(g => !g.deleted_at);
  
  const filteredGifts = activeGifts.filter(gift => {
    const matchesSearch = (gift.name || '').toLowerCase().includes(search.toLowerCase()) || 
                          (gift.event_name || '').toLowerCase().includes(search.toLowerCase()) ||
                          (gift.notes || '').toLowerCase().includes(search.toLowerCase());
    
    const matchesRel = selectedRel === 'Tất cả' || gift.relationship === selectedRel;
    
    // Event filter logic (handle 'Khác' type)
    let matchesEvent = true;
    if (selectedEvent !== 'Tất cả sự kiện') {
      if (selectedEvent === 'Khác') {
        const standardTypes = ['Đám cưới', 'Đám hiếu', 'Thăm ốm', 'Tân gia'];
        matchesEvent = gift.event_type && !standardTypes.includes(gift.event_type);
      } else {
        matchesEvent = gift.event_type === selectedEvent;
      }
    }
    ...
    <View style={styles.giftCard}>
      <View style={styles.giftHeader}>
        <View style={styles.giftNameWrapper}>
          ...
          <Text style={styles.giftRel}>{item.relationship} • {item.event_type}</Text>
        </View>
        <Text style={styles.giftAmount}>{formatVND(item.amount)}</Text>
      </View>

      <View style={styles.giftDetails}>
        <Text style={styles.giftMeta}>Sự kiện: <Text style={styles.metaValue}>{item.event_name || item.event_type}</Text></Text>
        <Text style={styles.giftMeta}>Ngày mừng: <Text style={styles.metaValue}>{formatDate(item.date)}</Text></Text>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.filterSection}>
        ...
        <TextInput 
          ...
          placeholder="Tìm người nhận, tên sự kiện, ghi chú..."
          value={search}
        ...
        {/* Event types filter scrollable row */}
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.relScroll}>
          {EVENT_TYPES.map((ev) => (
            <TouchableOpacity 
              key={ev} 
              style={[styles.filterChip, selectedEvent === ev && styles.activeEventChip]}
              onPress={() => { setSelectedEvent(ev); setPage(1); }}
            >
              <Text style={[styles.chipText, selectedEvent === ev && styles.activeEventChipText]}>{ev}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      </View>
      ...
      <FlatList 
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>Không tìm thấy khoản tiền mừng nào khớp bộ lọc.</Text>
          </View>
        }
      />
