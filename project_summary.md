...
| **Phiên bản hiện tại** | **v4.3.187** |
| **v4.3.187** | ✅ **Nâng Cấp Gộp Giao Dịch Quỹ 3-Way Merge, Tự Động Đồng Bộ Khi Có Mạng & Chống FOUC Theme (v4.3.187)**: (1) Chuyển đổi cơ chế gộp `fundTransactions` và `familyFunds` sang 3-way Item Merge (`mergeLists` theo `id` và `updated_at`), giải quyết triệt để lỗi ghi đè mất giao dịch khi 2 vợ chồng cùng ghi chép đồng thời; (2) Tích hợp `window.addEventListener('online')` tự động kích hoạt đồng bộ Supabase ngay khi thiết bị kết nối lại Wi-Fi/4G; (3) Bổ sung inline theme script trong `<head>` của `index.html` loại bỏ hoàn toàn hiện tượng chớp sáng giao diện (FOUC flash) khi tải trang. |
| **v4.3.186** | ✅ **Vá Lỗ Hổng Bảo Mật, Đồng Bộ Sự Kiện Âm Lịch & Tối Ưu Card Isolation (v4.3.186)**: (1) Tích hợp DOMPurify làm sạch mã Markdown chống tấn công Stored XSS / Prompt Injection trong Hồ sơ y tế; (2) Xác thực whitelist giao thức (`https:`, `http:`, `data:image:`) và escape HTML toàn diện cho URL ảnh trong WeLove; (3) Tích hợp sự kiện âm lịch gia đình (`lunarEvents`) vào state mã hóa AES-256 E2EE, tự động đồng bộ Supabase và sao lưu/phục hồi đa thiết bị; (4) Tách biệt hoàn toàn độc lập các module theo Card Isolation Rule (chuyển `callGeminiTextAPI` và helper về core `app.js`, chuẩn hóa model AI `gemini-2.5-flash`/`gemini-2.0-flash` loại bỏ 404); (5) Bổ sung `am-lich.css` và toàn bộ JS module vào Cache Shell PWA, tối ưu cache on-demand file âm thanh 5.9MB. |
| **v4.3.185** | ✅ **Nâng Kích Thước Chữ Lịch Vạn Niên (v4.3.185)**: Tăng kích thước font chữ cho các thành phần chính của Lịch Vạn Niên (Week header, Solar/Lunar day numbers, Can Chi, Tiết Khí, Hướng xuất hành, Việc nên làm/kiêng kỵ) giúp hiển thị to rõ ràng, cải thiện tối đa độ dễ đọc và giao diện trông cao cấp hơn trên cả PC và di động. |
| **v4.3.184** | ✅ **Sửa Nút Chọn Ngày Trên PC & Chú Thích Loại Lịch (v4.3.184)**: Khắc phục lỗi nút Chọn ngày không hoạt động trên máy tính bằng cách loại bỏ kiểm tra click target trùng với input ẩn (do input phủ kín nút nên click target luôn là input). Ở panel chi tiết bên phải, bổ sung thêm hậu tố `(Dương lịch)` và `(Âm lịch)` vào sau các tiêu đề ngày tương ứng để tăng độ trực quan. |
| **v4.3.183** | ✅ **Tích Hợp Nhắc Nhở Sự Kiện Âm Lịch & 12 Giờ Cát Hung (v4.3.183)**: Tích hợp sự kiện âm lịch (Ý tưởng 1): tự động hiển thị ngày lễ Tết âm lịch cổ truyền (Tết Đoan Ngọ, Vu Lan, Trung Thu, Tết Nguyên Đán, Rằm, Mùng 1...) và cho phép thêm/xóa ngày giỗ, sinh nhật âm lịch gia đình (lưu `localStorage`); hiển thị chấm chỉ báo màu sắc (đỏ cho gia đình, vàng cho ngày lễ) dưới ô ngày. Tích hợp 12 giờ Hoàng đạo/Hắc đạo (Ý tưởng 2): hiển thị danh sách toàn bộ 12 giờ trong ngày gắn nhãn 'Cát' (xanh) / 'Hung' (xám) trực quan, chi tiết thay thế cho danh sách chỉ có giờ hoàng đạo trước đây. |
