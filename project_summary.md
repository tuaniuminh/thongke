...
| **v4.3.187** | ✅ **Sửa Tên Ứng Dụng Khởi Chạy WiX & Đồng Bộ Theme Sáng Cửa Sổ PC (v4.3.187)**: Thay thế tường minh chuỗi `[ProductName]` thành `FamiLife` tại mục `LaunchApp` và toàn bộ các chuỗi trong `vi-VN.wxl` để khắc phục lỗi không render placeholder ở checkbox hoàn tất cài đặt. Bổ sung cấu hình `"theme": "Light"` trong `tauri.conf.json` giúp viền và thanh tiêu đề cửa sổ Windows hiển thị đồng bộ theo gam màu sáng, hài hòa với giao diện ứng dụng. |
