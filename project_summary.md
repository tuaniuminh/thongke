...
| **v4.3.120** | ✅ **Sửa Triệt Để Lỗi Tự Kết Nối Lại Sau Khi Hủy Gia Đình (v4.3.120)**: Tìm ra 2 nguyên nhân gốc rễ: (1) Khi hủy liên kết, `performSync` đọc `remoteData.spouseEmail` từ bản lưu cũ trên Supabase và toán tử `|| !state.spouseEmail` tự động phục hồi lại `spouseEmail` vào local state của người hủy. Fix: Sửa khối gộp spouse trong [`app.js`](file:///c:/Users/PC%20VIP/Documents/Thong-ke/src/core/app.js) chỉ gộp khi `spouseStatus === 'accepted'` và xóa sạch nếu `spouseStatus === 'left'` hoặc hủy liên kết. (2) Khi vợ mở app sau đó, `checkForSharedFamilyFund` trong [`ket-noi.js`](file:///c:/Users/PC%20VIP/Documents/Thong-ke/src/features/ket-noi-gia-dinh/ket-noi.js) không nhận diện được việc chồng đã hủy (do mã ghép đôi rỗng). Fix: Mở rộng thuật toán tự động nhận diện hủy liên kết ở phía vợ khi dòng của chồng có `remoteSpouseStatus === 'left'` hoặc `!remoteSpouseEmail`, ép thiết bị vợ tự dọn sạch local & Supabase. Nâng phiên bản toàn hệ thống sang `?v=4.3.120`. |
