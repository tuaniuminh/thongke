...
| **Phiên bản hiện tại** | **v4.3.133** |
| **v4.3.133** | ✅ **Sửa Triệt Để Lỗi Hiển Thị Dư Tab Và Đổi Tên Logo Thành FamiLife Khi Tải Lại Trang (F5) (v4.3.133)**: (1) **Ẩn Mặc Định Tab Phụ Trên HTML**: Bổ sung `style="display: none;"` cho các mục `data-nav="welove"` và `data-nav="health"` trong static markup `index.html`, ngăn ngừa hiện tượng lộ tab khi JS chưa khởi tạo xong. (2) **Đồng Bộ Navbar Khi Tải Trang Theo Hash**: Đảm bảo `updateSidebarNavVisibility(tabId)` luôn được thực thi trong `switchTab` và `enterApp` kể cả khi tab đã ở trạng thái active. Đảm bảo tên logo luôn hiển thị đúng *"Thu Chi Đối Ngoại"* và các tab hiển thị chính xác 100% khi nhấn F5 tải lại trang tại URL `#tongquan`. Nâng phiên bản toàn hệ thống sang `?v=4.3.133`. |
| **v4.3.132** | ✅ **Tích Hợp Tùy Chọn Bố Cục Trang Chủ Bento Grid 3 Cột (Máy Tính) Trong Cài Đặt (v4.3.132)**: Thêm tùy chọn *"Bố cục Trang chủ (Máy tính)"* trong tab Cài Đặt cho phép người dùng linh hoạt chuyển đổi giữa **📄 Xếp hàng dọc 1 cột (Mặc định)** và **🍱 Bento Grid 3 cột (Thử nghiệm 🚀)**. Khi bật chế độ Bento Grid, các card ngoài trang chủ trên màn hình máy tính (`min-width: 992px`) được tự động căn dàn ngang 3 cột hiện đại, tối ưu diện tích hiển thị và bổ sung hiệu ứng hover 3D nâng nhẹ sang trọng. Nâng phiên bản toàn hệ thống sang `?v=4.3.132`. |
