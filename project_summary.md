...
| **v4.4.8** | ✅ **Sửa Triệt Để Lỗi Lệch 100px ScrollTop & Đảm Bảo Cuộn Đầu Trang Kể Cả Khi Tab Đã Active (v4.4.8)**: (1) Sửa `.modal-overlay` trong [`src/assets/css/style.css`](file:///c:/Users/PC VIP/Documents/Thong-ke/src/assets/css/style.css) bỏ `top: -100px; height: calc(100% + 100px); padding-top: 100px;` (nguyên nhân chính làm phình `html.scrollHeight` lên 1198px và ép `html.scrollTop = 100px`). (2) Bổ sung `window.scrollTo(0, 0)` và `setTimeout 50ms` trực tiếp vào khối return sớm khi `state.activeTab === tabId` tại `switchTab` trong [`src/core/app.js`](file:///c:/Users/PC VIP/Documents/Thong-ke/src/core/app.js). Giải quyết dứt điểm hiện tượng trôi vị trí trang khi nhấp lại tab. Nâng phiên bản toàn hệ thống sang `?v=4.4.8`. |
