...
| **v4.3.114** | ✅ **Sửa triệt để lỗi kết nối lại gia đình — Chặn checkForSharedFamilyFund khi đang hủy liên kết (v4.3.114)**: Qua bug detector log phát hiện nguyên nhân gốc rễ: hàm polling `checkForSharedFamilyFund` chạy song song trong khi đang hủy liên kết và ghi đè `spouseStatus = 'accepted'` trở lại (dòng log: `Spouse accepted invitation. Updating spouseStatus to accepted.` xuất hiện giữa Unlink Step 1 và Step 2). Fix: Thêm guard `if (window._isUnlinking) return;` vào đầu hàm `checkForSharedFamilyFund` trong [`ket-noi.js`](file:///c:/Users/PC%20VIP/Documents/Thong-ke/src/features/ket-noi-gia-dinh/ket-noi.js) để dừng toàn bộ quá trình polling khi đang trong tiến trình hủy liên kết. Nâng cấp phiên bản toàn hệ thống và cache-busting sang `?v=4.3.114`. |
| **v4.3.113** | ✅ **Nâng cấp Bug Detector & Sửa lỗi performSync ghi đè hủy liên kết (v4.3.113)**: (1) Bổ sung cờ `window._isUnlinking` được bật trước khi thực hiện hủy liên kết trong [`ket-noi.js`](file:///c:/Users/PC%20VIP/Documents/Thong-ke/src/features/ket-noi-gia-dinh/ket-noi.js) và tắt đi sau khi hoàn tất. (2) Sửa hàm `performSync` trong [`app.js`](file:///c:/Users/PC%20VIP/Documents/Thong-ke/src/core/app.js) để kiểm tra cờ `window._isUnlinking`, bỏ qua toàn bộ việc ghi đè thông tin spouse từ máy chủ khi đang trong quá trình hủy liên kết. (3) Bổ sung hệ thống debug log `[BUG DETECTOR]` chi tiết tại tất cả các điểm xử lý spouse trong `performSync` và handler hủy liên kết. (4) Nâng cấp phiên bản toàn hệ thống và cache-busting sang `?v=4.3.113`. |
