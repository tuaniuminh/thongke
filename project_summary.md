...
| **Phiên bản hiện tại** | **v4.3.130** |
| **v4.3.130** | ✅ **Sửa Màu Tiêu Đề Card Trên Navbar Giao Diện Máy Tính (#sidebarLogoText) (v4.3.130)**: Áp dụng lớp CSS gradient theme tương ứng (`theme-thuchi`, `theme-health`, `theme-fund`, `theme-welove`, `theme-settings`) cho `sidebarLogoText` khi chuyển tab trên giao diện Máy tính. Đảm bảo dòng chữ tiêu đề card ở góc trái Navbar Desktop (ví dụ: *"Thu Chi Đối Ngoại"*, *"Hồ Sơ Y Tế"*, *"Quỹ Gia Đình"*) luôn đổi màu gradient rực rỡ đồng bộ 100% với giao diện Điện thoại và đúng yêu cầu. Nâng phiên bản toàn hệ thống sang `?v=4.3.130`. |
| **v4.3.129** | ✅ **Xóa Tiêu Đề Trùng Lặp Phía Dưới Navbar Trong Tab WeLove (v4.3.129)**: Trả về trạng thái chuẩn `title.innerText = ''` và `subtitle.innerText = ''` cho Desktop top-header trong tab WeLove, loại bỏ hoàn toàn tiêu đề lặp lại không cần thiết ở phía dưới navbar. Nâng phiên bản toàn hệ thống sang `?v=4.3.129`. |
| **v4.3.128** | ✅ **Sửa Triệt Để Màu Tiêu Đề Card Trên Navbar Giao Diện Máy Tính (Desktop Header) (v4.3.128)**: Bổ sung các thuộc tính `background-clip: text !important`, `-webkit-background-clip: text !important` và `color: transparent !important` chuẩn cross-browser cho `#currentTabTitle`. Nâng phiên bản toàn hệ thống sang `?v=4.3.128`. |
| **v4.3.127** | ✅ **Sửa Triệt Để Lỗi Lệch Giao Diện Card Kỷ Niệm, Khóa Cuộn iOS 16.6+ & Phủ Màu Tiêu Đề Card/Navbar (v4.3.127)**: (1) **Sửa Triệt Để Lỗi Lệch Giao Diện**: Khôi phục lại bộ CSS `.memory-card` chuẩn ban đầu (padding `3rem 2rem`, `max-width: 600px`), phân tách hoàn toàn với `.welove-card` để căn giữa đối xứng chuẩn 100% hình ảnh. (2) **Khóa Cuộn Nền Chống Trôi Trên iOS 16.6+**: Tích hợp cơ chế iOS Safari Scroll Lock bằng `position: fixed` và `touchmove` non-passive preventDefault trên overlay modal Quản Lý Album Ảnh Kỷ Niệm. (3) **Đồng Bộ Màu Viền & Tiêu Đề Card/Navbar**: Mở rộng màu viền đặc trưng của các card ngoài trang chủ ra cả giao diện Máy tính và Điện thoại, đồng thời cập nhật màu chữ tiêu đề trên Navbar tương ứng (chữ Cài đặt màu Đen, WeLove giữ nguyên). Nâng phiên bản toàn hệ thống sang `?v=4.3.127`. |
| **v4.3.126** | ✅ **Tối Ưu Nền Tab Card WeLove, Chặn Cuộn Modal Album Ảnh & Phủ Màu Gradient Tiêu Đề Card (v4.3.126)**: (1) **Đồng Bộ Nền Tab Card WeLove**: Bổ sung bộ CSS Glassmorphism cho `.welove-card`, giúp màu nền 2 tab "Lời nhắc" & "Quản lý" mượt mà đồng bộ với tab "Kỷ niệm". (2) **Khóa Cuộn Nền & Nút Đóng Modal Album**: Tự động khóa cuộn trang khi mở bảng *Quản lý Album Ảnh Kỷ Niệm*, tháo bỏ nút đóng ở dưới và đặt nút `✕` tròn lên góc trên phải của modal. (3) **Phủ Màu Gradient Tiêu Đề Card**: Bổ sung lớp CSS Gradient theo từng chủ đề card cho tiêu đề trên Navbar Desktop (`#currentTabTitle`) và Mobile (`.mobile-navbar-title`). Nâng phiên bản toàn hệ thống sang `?v=4.3.126`. |
