...
| **Phiên bản hiện tại** | **v4.3.192** |
| **v4.3.192** | ✅ **Việt Hóa Ngôn Ngữ Gốc WiX (LCID 1066) & Tự Động Ẩn Titlebar Khi Xem Ảnh Toàn Màn Hình (v4.3.192)**: Cấu hình mã vùng ngôn ngữ WiX chính thức sang tiếng Việt `Culture="vi-VN"` / `TauriLanguage="1066"` trong `tauri.conf.json` và `vi-VN.wxl`, giúp tất cả hộp thoại tiến trình gỡ cài đặt hệ thống của Windows Settings hiển thị tiếng Việt hoàn toàn. Bổ sung cơ chế tự động ẩn thanh tiêu đề desktop khi mở Lightbox xem ảnh toàn màn hình (`body.lightbox-open`), đảm bảo ảnh xem ở WeLove luôn tràn viền 100% không bị lẹm hoặc che khuất. |
| **v4.3.191** | ✅ **Sửa Cơ Chế Tự Động Xóa AppData WiX Sang Deferred Elevated (v4.3.191)**: Khắc phục triệt để lỗi không xóa sạch dữ liệu người dùng khi gỡ cài đặt do biến `$env:LOCALAPPDATA` bị trỏ vào tài khoản hệ thống khi MSI chạy quyền Admin. Chuyển CustomAction sang chế độ `Execute="deferred" Impersonate="no"` (chạy dưới quyền SYSTEM) và quét trực tiếp qua `C:\Users\*\AppData\Local\com.familife.thongke` để đảm bảo 100% session Supabase, mã PIN và bộ nhớ đệm của toàn bộ người dùng trên máy tính đều được xóa sạch khi gỡ phần mềm. |
| **v4.3.190** | ✅ **Kích Hoạt withGlobalTauri & Phát Hiện Sớm Giao Diện Thanh Tiêu Đề Desktop (v4.3.190)**: Bổ sung `"withGlobalTauri": true` vào cấu hình `build` trong `tauri.conf.json` để hệ thống Tauri tự động inject đối tượng toàn cục `window.__TAURI__` vào WebView. Bổ sung script phát hiện sớm môi trường desktop và cơ chế retry khởi tạo, đảm bảo các nút Thu nhỏ, Phóng to/Khôi phục, Đóng (`—` `□` `✕`) của thanh tiêu đề tùy biến luôn hiển thị ngay lập tức khi mở ứng dụng trên máy tính. |
| **v4.3.189** | ✅ **Tích Hợp Thanh Tiêu Đề Cửa Sổ Tùy Biến Đổi Màu Đồng Bộ 100% (v4.3.189)**: Triển khai giao diện cửa sổ không viền (Frameless Window - `"decorations": false`) cùng thanh tiêu đề tùy biến HTML/CSS (`#tauriTitlebar`) có gắn `data-tauri-drag-region`, đầy đủ nút Thu nhỏ (`—`), Phóng to/Khôi phục (`□`/`❐`), Đóng (`✕`) và hỗ trợ double-click phóng to. Thanh tiêu đề sử dụng biến CSS toàn cục, tự động đổi màu mượt mà 100% khi chuyển đổi chế độ Sáng $\leftrightarrow$ Tối (Light/Dark Mode). |
| **v4.3.188** | ✅ **Tự Động Xóa Sạch 100% Dữ Liệu Ứng Dụng Khi Gỡ Cài Đặt WiX MSI (v4.3.188)**: Bổ sung custom action `CleanupUserData` vào template WiX (`src-tauri/wix/main.wxs`) được thực thi tự động khi gỡ bỏ phần mềm (`REMOVE="ALL"`), giúp quét và xóa sạch hoàn toàn thư mục `%LOCALAPPDATA%\com.familife.thongke` (toàn bộ session Supabase, mã PIN, cache dữ liệu). Người dùng không còn phải xóa thủ công khi gỡ ứng dụng. |
