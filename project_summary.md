...
| **Phiên bản hiện tại** | **v4.6.9** |
| **v4.6.9** | ✅ **Sửa Triệt Để Lỗi Dữ Liệu Cục Bộ Tự Động Sửa Sai Vai Trò `spouseRole = husband` & Cho Phép Chuyển Sub-tab Lời Nhắc / Quản Lý Tức Thì (v4.6.9)**: (1) Khắc phục nguyên nhân gốc rễ do giá trị mặc định của `spouseRole` bị nạp nhầm thành `'wife'` trong `state` và `loadLocalState()` của [`src/core/app.js`](file:///c:/Users/PC VIP/Documents/Thong-ke/src/core/app.js). Bổ sung thuật toán tự phục hồi (self-healing state repair) tự động đưa vai trò về `'husband'` đối với Chủ tài khoản. (2) Sửa mã hủy kết nối trong [`src/features/ket-noi-gia-dinh/ket-noi.js`](file:///c:/Users/PC VIP/Documents/Thong-ke/src/features/ket-noi-gia-dinh/ket-noi.js). Giúp Chủ tài khoản Chồng chuyển giữa các sub-tab **Kỷ niệm**, **Lịch nhắc** và **Quản lý** tức thì không cần F5 nạp lại trang. Nâng phiên bản toàn hệ thống sang `?v=4.6.9`. |
| **v4.6.8** | ✅ **Sửa Lỗi Tài Khoản Chồng (Admin) Bị Chặn Quyền Chuyển Sub-tab Lời Nhắc & Quản Lý Trong Card WeLove (v4.6.8)**: (1) Sửa điều kiện điều hướng routing trong [`src/core/app.js`](file:///c:/Users/PC VIP/Documents/Thong-ke/src/core/app.js) (`switchTab`, `navigateToTab`, `updateSidebarNavVisibility`) bổ sung `state.spouseRole === 'wife'` đảm bảo cờ `viewingSharedFund` chỉ chặn truy cập đối với tài khoản Vợ (Khách xem chung), hoàn trả 100% quyền truy cập sub-tab `welove-admin` và `welove-settings` cho Chủ tài khoản Chồng (Admin). (2) Cập nhật cờ kiểm tra trong [`src/features/we-love/we-love.js`](file:///c:/Users/PC VIP/Documents/Thong-ke/src/features/we-love/we-love.js) và [`src/features/ket-noi-gia-dinh/ket-noi.js`](file:///c:/Users/PC VIP/Documents/Thong-ke/src/features/ket-noi-gia-dinh/ket-noi.js). Nâng phiên bản toàn hệ thống sang `?v=4.6.8`. |
