...
| **v4.3.126** | ✅ **Tối Ưu Nền Tab Card WeLove, Chặn Cuộn Modal Album Ảnh & Phủ Màu Gradient Tiêu Đề Card (v4.3.126)**: (1) **Đồng Bộ Nền Tab Card WeLove**: Bổ sung bộ CSS Glassmorphism (`background: var(--glass-bg)`, `backdrop-filter`, `border`, `box-shadow`) cho `.welove-card`, giúp màu nền 2 tab "Lời nhắc" & "Quản lý" đậm đà và mượt mà đồng bộ 100% với tab "Kỷ niệm". (2) **Khóa Cuộn Nền & Nút Đóng Modal Album**: Tự động khóa cuộn trang (`overflow: hidden`) khi mở bảng *Quản lý Album Ảnh Kỷ Niệm*, tháo bỏ nút đóng ở dưới và đặt nút `✕` tròn lên góc trên phải của modal. (3) **Phủ Màu Gradient Tiêu Đề Card**: Bổ sung lớp CSS Gradient theo từng chủ đề card cho tiêu đề trên Navbar Desktop (`#currentTabTitle`) và Mobile (`.mobile-navbar-title`). Nâng phiên bản toàn hệ thống sang `?v=4.3.126`. |
