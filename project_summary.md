...
| **Phiên bản hiện tại** | **v4.3.195** |
| **v4.3.195** | ✅ **Sửa Lỗi Relative Sidebar Top Offset & Nâng Cấp BUG DETECTOR (v4.3.195)**: Tìm ra nguyên nhân gốc rễ: ở WeLove và Hồ Sơ Y Tế, navbar được cấu hình `position: relative` (để cuộn tự nhiên trên màn hình lớn), khi gặp quy tắc `top: 32px` của bản desktop đã bị dịch xuống 32px tạo ra khoảng trống trắng phía trên. Đã chỉnh sửa `top: 0 !important` cho các card này, loại bỏ 100% khoảng trống thừa. Đồng thời nâng cấp hệ thống `[BUG DETECTOR]` in chi tiết kích thước layout tại mỗi lần chuyển tab. |
| **v4.3.194** | ✅ **Loại Bỏ Khoảng Trống Thừa (Top-Header) Tại WeLove & Hồ Sơ Y Tế Trên Desktop (v4.3.194)**: Xử lý triệt để khoảng trống thụt lề ở card WeLove và Hồ Sơ Y Tế. Ẩn hoàn toàn thẻ `top-header` rỗng khi ở chế độ WeLove (`body.active-tab-welove`) và thu gọn khoảng cách đệm trên Desktop, giúp các card WeLove và Hồ Sơ Y Tế nằm ngay sát dưới thanh điều hướng, chuẩn xác và đồng nhất 100% với bản PWA. |
