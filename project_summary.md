...
| **v4.3.143** | ✅ **Giữ Nguyên Background Cho Mọi Sub-tab We Love & Giảm Padding Mobile (v4.3.143)**: Cập nhật CSS để duy trì hình nền màu hồng (light mode) và màu tối (dark mode) của We Love khi chuyển sang tab Lịch nhắc (`active-tab-welove-admin`) hoặc Quản lý (`active-tab-welove-settings`), đồng thời giảm padding-top và margin-top của tiêu đề trên mobile để kéo tên cặp đôi lên sát Navbar ở mục We Love. Nâng phiên bản toàn hệ thống sang `?v=4.3.143`. |
