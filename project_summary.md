...
| **v4.6.2** | ✅ **Phát Hiện & Xóa Thẻ Đóng `</div>` Dư Thừa Đóng Nhầm `main-content` Khiến `tab-vehicle` Bị Đẩy Xuống Mốc 740px (v4.6.2)**: Nhờ log chẩn đoán sâu `[BUG DETECTOR]` ở v4.6.1 phát hiện `tab-vehicle` bị trôi xuống tận vị trí `top = 740px`, phát hiện ra 1 thẻ đóng `</div>` dư thừa ở dòng 1547 trong [`index.html`](file:///c:/Users/PC VIP/Documents/Thong-ke/index.html) làm đóng sớm container `.main-content`. Xóa bỏ thẻ `</div>` dư thừa đưa `tab-vehicle` trở lại nằm trực tiếp bên trong `.main-content` ở vị trí `top = 104px` liền sát dưới tiêu đề chính. Nâng phiên bản toàn hệ thống sang `?v=4.6.2`. |
| **v4.6.1** | ✅ **Nâng Cấp Hệ Thống Debug Log `runLayoutDiagnostics()` Quét Sâu Chi Tiết Toàn Bộ Cây DOM Trực Thuộc `tab-vehicle` (v4.6.1)**: Áp dụng Quy tắc 4 (Bug Detector Rule), nâng cấp sâu hệ thống log khi nhấn nút con bọ màu hồng để quét và in vết toàn bộ kích thước, vị trí lề `marginTop`, `paddingTop`, `top`, `display` của từng thẻ con và thẻ cháu nằm trong `main-content` và `tab-vehicle` trong [`src/core/app.js`](file:///c:/Users/PC VIP/Documents/Thong-ke/src/core/app.js). Nâng phiên bản toàn hệ thống sang `?v=4.6.1`. |
| **v4.6.0** | ✅ **Gỡ Bỏ Class `card-panel` Khỏi Khung Xe Rỗng & Reset Margin/Padding 0px Cho `#tab-vehicle` (v4.6.0)**: (1) Gỡ bỏ class `card-panel` khỏi `div#vehicleEmptyNotice` trong [`index.html`](file:///c:/Users/PC VIP/Documents/Thong-ke/index.html) để loại bỏ hoàn toàn thuộc tính `min-height: 120px` và `margin-bottom: 2rem` khi ẩn. (2) Khai báo quy tắc CSS `#tab-vehicle, #vehicleMainContentBox { margin: 0 !important; padding: 0 !important; }` trong [`src/assets/css/style.css`](file:///c:/Users/PC VIP/Documents/Thong-ke/src/assets/css/style.css) ép khối danh sách xe hiển thị liền sát ngay dưới thanh tiêu đề chính, triệt tiêu 100% khoảng khoảng trống thừa. Nâng phiên bản toàn hệ thống sang `?v=4.6.0`. |
