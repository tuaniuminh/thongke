...
| **Phiên bản hiện tại** | **v4.3.148** |
| **v4.3.148** | ✅ **Tích Hợp 4 Kiểu Hiển Thị Album Ảnh Trên Desktop & Lựa Chọn Bố Cục (v4.3.148)**: Cho phép chuyển đổi 5 bố cục Album ảnh trên Desktop trong tab Thiết lập gồm: Trượt ngang (`slider`), Bảng ghim Polaroid (`polaroid`), Vòng quay 3D (`carousel3d`), Rạp chiếu phim (`theater`) và Lưới so le Masonry (`grid`). Đồng bộ state `weLoveAlbumLayoutDesktop` toàn hệ thống và bổ sung đầy đủ CSS chất lượng cao cho từng layout. Nâng phiên bản toàn hệ thống sang `?v=4.3.148`. |
| **v4.3.147** | ✅ **Lồng Trái Tim Lớn Vào Giữa Tên Desktop & Làm Nổi Bật Dấu Mốc Năm (v4.3.147)**: Tích hợp trái tim lớn đang đập `💝` (`.heart-pulsing`) vào chính giữa hai tên trên desktop để tạo thành một hàng ngang cân đối; ẩn trái tim độc lập phía trên card kỷ niệm và khung tên cũ trên desktop; làm nổi bật chữ hiển thị mốc năm (`1 năm`, `2 năm`...) bằng cách in đậm và chuyển sang màu hồng cherry trên thanh tiến trình. Nâng phiên bản toàn hệ thống sang `?v=4.3.147`. |
| **v4.3.146** | ✅ **Chuyển Tên Cặp Đôi Vào Trong Card Kỷ Niệm Trên Desktop (v4.3.146)**: Trên máy tính (Desktop), ẩn chiếc khung glassmorphism tên độc lập `.couple-names-header` ở trên đầu, thay vào đó hiển thị tên cặp đôi trực tiếp bên trong card kỷ niệm `.memory-card` tại vị trí của tiêu đề cũ H2 `.memory-title`, giúp giao diện nằm thẳng hàng và bố cục cực kỳ gọn gàng. Nâng phiên bản toàn hệ thống sang `?v=4.3.146`. |
| **v4.3.145** | ✅ **Cập Nhật Cột Mốc Kỷ Niệm Theo Năm & Layout Di Động & Rút Timeout Check Update (v4.3.145)**: Thay đổi cột mốc kỷ niệm trong We Love sang tính theo năm (1 năm, 2 năm, 3 năm...) thay vì ngày; di chuyển dòng hiển thị số ngày còn lại xuống dưới bên phải thanh tiến trình chỉ riêng trên giao diện di động bằng CSS Flexbox `order`; và giảm thời gian hiển thị kết quả check update (Đã mới nhất / Lỗi kết nối) trên Badge từ 3 giây xuống 1 giây. Nâng phiên bản toàn hệ thống sang `?v=4.3.145`. |
| **v4.3.144** | ✅ **Tự Động Nhận Diện Độ Dài Tên Cặp Đôi & Thu Hẹp Khoảng Cách Layout & Hiệu Ứng Kiểm Tra Cập Nhật Độc Đáo (v4.3.144)**: Tự động gán các class kích thước (`name-short` - `1.6rem` cho <=2 từ, `name-medium` - `1.3rem` cho 3 từ, `name-long` - `1.1rem` cho >=4 từ) trên mobile để đảm bảo cân đối; giảm `margin-bottom` của `.mobile-navbar` xuống `0.5rem` khi ở mục We Love để đẩy tên cặp đôi lên sát Navbar; và loại bỏ Toast khi click kiểm tra cập nhật thủ công, thay bằng hiệu ứng xoay biểu tượng và hiển thị trực tiếp kết quả (màu sắc/text) ngay tại ô số phiên bản. Nâng phiên bản toàn hệ thống sang `?v=4.3.144`. |
