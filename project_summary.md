...
| **v4.3.186** | ✅ **Việt Hóa Toàn Diện Bộ Cài Đặt & Gỡ Bỏ Windows MSI (v4.3.186)**: Cập nhật đầy đủ bản dịch tiếng Việt chuẩn xác cho tất cả các hộp thoại trong trình cài đặt WiX MSI (`vi-VN.wxl`), bao gồm: màn hình đang cài đặt (`ProgressDlgTitleInstalling`/`TextInstalling`), màn hình gián đoạn/hủy (`UserExit`), hộp thoại xác nhận hủy (`CancelDlg`), các bước gỡ bỏ/sửa chữa ứng dụng (`VerifyReadyDlgRemove`/`Repair`, `ProgressDlgRemoving`/`Repairing`), báo lỗi (`FatalError`), xung đột ứng dụng (`FilesInUse`) và duyệt thư mục (`BrowseDlg`). |
