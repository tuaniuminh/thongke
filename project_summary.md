...
| **v4.3.133** | ✅ **Sửa Triệt Để Lỗi Không Nâng Cấp Được Phiên Bản Do Mất Đồng Bộ Số Version Giữa index.html và version.json (v4.3.133)**: Phát hiện nguyên nhân khiến thông báo *"Đã có bản cập nhật mới (v4.3.132)"* hiện liên tục ở góc màn hình nhưng bấm vào tải lại vẫn hiển thị `Ver 4.3.131`: do `index.html` chưa được cập nhật số phiên bản đồng loạt trong thẻ `<script>window.APP_VERSION</script>`, importmap và Service Worker registration. Đã đồng bộ 100% tất cả các file về phiên bản `v4.3.133`, triệt tiêu hoàn toàn thông báo cập nhật lặp lại. Nâng phiên bản toàn hệ thống sang `?v=4.3.133`. |
